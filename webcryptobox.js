...
export const sign = async (privateKey, message, algorithm= {name: "ECDSA", hash: {name: "SHA-384"}}) => {
  ...
  await window.crypto.subtle.sign(algorithm, signingKey, message);
}

export const verify = async (publicKey, message, signature, algorithm={name: "ECDSA", hash: {name: "SHA-384"}}) => {
  await window.crypto.subtle.verify(algorithm, publicKey, signature, message);
}
