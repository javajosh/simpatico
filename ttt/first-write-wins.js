...
function combineFormData(fd1, fd2){
  ...
  for (let [key, value] of fd1.entries()) result.append(key, value);
  for (let [key, value] of fd2.entries()) result.append(key, value);
  return result;
}
...
const table = (arr, th, tr, td) => w('table', tr(arr[0],th) + arr.splice(1).map(a => tr(a, td)).join(''));
...
function transpose(arr){
  ...
  for (let i = 0; i< width; i++){
    col = [];
    for (let j = 0; j < len; j++) col.push(arr[j][i]);
    cols.push(col);
  }
  return cols;
}
...
function randomlyDeleteProperties(obj){
  ...
  if (keepSet.length === 0) return obj;
